...
% baselineSelect = [1,1,0,1,0]; %Proposed NoSL SV oracle  w/opp
% baselineSelect = [1,0,0,0,0]; %Proposed NoSL SV oracle  w/opp

% baselineSelect = [0,0,0,0,1]; %Proposed NoSL SV oracle  w/opp
...
for nsim = [3,6]
    ...
    rmsePhaseI     = zeros(length(tauGT),1);
    rmseProposed   = zeros(length(tauGT),1);
    rmseNoSL       = zeros(length(tauGT),1);
    rmseSV         = zeros(length(tauGT),1);
    rmseWOPP       = zeros(length(tauGT),1);
    rmseOracle     = zeros(length(tauGT),1);
...
%             inputParamtemp.gammaOmega = 1e+3;
    ...
    %     inputParam.gammaPrior = 1e+3;
%             inputParamtemp.gammaOmega = 1e+3;
            [~,stateWOPP(expIdx)] = JSLES4(yp,Phip,inputParamtemp);
            rmseWOPP(expIdx) = stateWOPP(expIdx).RMSEfinal;
        end
        %% NoAid
%         if baselineSelect(5)
%             inputParamtemp.gg = x(:,:);
...
%             inputParamtemp.gammaOmega = 5e+2;
            ...
            [~,stateOracletemp(expIdx)] = JSLES4(yp,PhiGTO{expIdx},inputParamtemp);
...
%     semilogy(tauGT'*1e9,mean(RMSENoAid(:,:),2),'-s',LineWidth=1.5,Color = [220,20,60]/255);hold on;
    semilogy(tauGT'*1e9,mean(RMSEWOPP(:,:),2),'-+',LineWidth=1.5,Color = [250,128,114]/255);hold on;
...
xlabel('Time offset (ns)',Interpreter='latex',FontName='Times New Roman',FontSize=15);

nameStr = strcat('fig\figureTORMSE1','.fig');
savefig(nameStr);
...
str = strcat('results\',str,'mainTONonRobust','.mat');
...
save(str,...
    'RMSEProposed','RMSEWOPP','RMSENoSL','RMSEOracle',"tauGT");
