...
%         gammaPrior = mean(piL,'all')./mean( piL.*( ( (muPrior - m).*conj(muPrior - m) + 1./gammaV ) ),'all')*ones(N,numBS);
        ...
        if norm(x1-xold,'fro')/norm(xold,'fro') < 1e-2
            break;
        end
...
%             gammaPrior = ones(N,1)*mean(piL,1)./mean( piL.*( ( (muPrior - m).*conj(muPrior - m) + 1./gammaV ) ),1);
            gammaPrior = mean(piL,'all')./mean( piL.*( ( (muPrior - m).*conj(muPrior - m) + 1./gammaV ) ),'all')*ones(N,numBS);
            Lambda = ones(N,numBS)*max(mean(piL,'all'),0);
        end
    end
...
%     state.mu(t) = norm(muPrior,'fro');
    ...
    if norm(x1 - xoldOut,'fro')/norm(xoldOut,'fro') < 5e-3 && convBreaker
        break;
    end
    if t == EMiter
        break;
    ...
    if flagOG && t > 1
        ...
        for indBS = 1:numBS
            gx(indBS,:) = -2*(real( (y(:,indBS) - A(:,:,indBS)*x2(:,indBS))'*(derivP(indBS).Phidx.*repmat(x2(:,indBS).',size(y,1),1)) ) - ...
                real(diag( cov2(:,:,indBS)*A(:,:,indBS)'*derivP(indBS).Phidx).'));
            gy(indBS,:) = -2*(real( (y(:,indBS) - A(:,:,indBS)*x2(:,indBS))'*(derivP(indBS).Phidy.*repmat(x2(:,indBS).',size(y,1),1)) ) - ...
                real(diag( cov2(:,:,indBS)*A(:,:,indBS)'*derivP(indBS).Phidy).'));
        ...
        if numBS > 1
        jx = [jx(:,:,1);jx(:,:,2)];jy = [jy(:,:,1);jy(:,:,2)]; 
        end
        if 1
%             Dmax = min(abs([maxStep - deltaX,-maxStep - deltaX,maxStep - deltaY,-maxStep - deltaY]));
            Dmax = maxStep;
            alphaG = Dmax./max(abs([gx,gy]),[],'all')/5;
        ...
        if flagSL
            ...
            for indBS = 1:numBS
                ...
                jxU(:,indBS) = derivP(indBS).PhidxUAV*x2(:,indBS);
                jyU(:,indBS) = derivP(indBS).PhidyUAV*x2(:,indBS);
                jtau(:,indBS) = derivP(indBS).Phidtau*x2(:,indBS)/c0;
%                 fBase(indBS) = norm(y(:,indBS) - A(:,:,indBS)*x2(:,indBS),'fro')^2 + ...
%                     trace(A(:,:,indBS)*cov2(:,:,indBS)*A(:,:,indBS)');
%                 fBase(indBS) = real(fBase(indBS));
            end
            
            jxU = reshape(jxU,[],1); jyU = reshape(jyU,[],1); jtau = reshape(jtau,[],1);
            gxU = sum(gxU); gyU = sum(gyU); gzU = sum(gzU); gtau = sum(gtau);
        end

        if numBS > 1
        rr = [y(:,1) - A(:,:,1)*x2(:,1);y(:,2) - A(:,:,2)*x2(:,2)];
        else
        rr = [y(:,1) - A(:,:,1)*x2(:,1)];
        end
        if ~flagSL
            J = [jx,jy];
            deltaZeta = real(J'*J + lambdaLM*eye(size(J,2)))\real(J'*rr);
            stepX = deltaZeta(1:length(stepX),1)';stepY = deltaZeta(length(stepY)+1:2*length(stepY),1)';
        else
            J = [jx,jy,jxU,jyU,jtau];
            deltaZeta = real(J'*J + lambdaLM*diag(diag(J'*J)))\real(J'*rr);
            stepX = deltaZeta(1:length(stepX),1)';stepY = deltaZeta(length(stepY)+1:2*length(stepY),1)';
            stepxU = deltaZeta(2*length(stepY)+1,1);
            stepyU = deltaZeta(2*length(stepY)+2,1);
            steptau = deltaZeta(2*length(stepY)+3,1)/c0;
            stepzU = 0;
        end

        clear PhipSearchtmp derivPSearchtmp 
    ...
    %                 sMGParam.symbol = ones(length(pilotSubcarrInd),1);
            ...
            if flagSL
                termSL = (armijoSigma*armijoBeta2*armijoRho^(iterArmijo-1)*(gxU*stepxU + gyU*stepyU + gzU*stepzU)) +...
                    armijoSigma*armijoBeta3*armijoRho^(iterArmijo-1)*(gtau*steptau);
            else
                termSL = 0;
            end
