function [x1,state] = EMBivamp(y,A,inputParam)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Hddelay = inputParam.sMDeriv.PhiddelaywLoS;

% Hdxdx  = inputParam.sMDeriv.Phidxdx;
% Hdxdx2 = inputParam.sMDeriv.Phidxdx2;
% Hdddd  = inputParam.sMDeriv.Phidddd;
% Hdddd2 = inputParam.sMDeriv.Phidddd2;
...
%     gamma2 = 100;
    ...
    %         piL = 1./(1 + (1-Lambda)./Lambda.*exp(-L));
        ...
        if any(isnan(x1))
            error('NaN occurs in x1 estimate!');
        end
     %% M step
    ...
    %     muPrior = m;
        if norm(x1 - xoldOut,'fro')^2/norm(xoldOut,'fro')^2 < 1e-8 && convBreaker
            break;
        end
    
    
    %     Hddelay = sMDeriv.PhiddelaywLoS/norm(A,2);
        
    %     Hdxdx  = sMDeriv.Phidxdx/norm(A,2);
    %     Hdxdx2 = sMDeriv.Phidxdx2/norm(A,2);
    %     Hdddd  = sMDeriv.Phidddd/norm(A,2);
    %     Hdddd2 = sMDeriv.Phidddd2/norm(A,2);
    
    
        %% record EM's M-step result
    %     state.mu(t) = norm(muPrior,'fro');
        state.gammaP(t,tt) = Lambda./mean( piL.* ( (muPrior - m).*conj(muPrior - m) + 1./gammaV ) );
        state.lambda(t,tt) = Lambda;
        state.gammaOmega(t,tt) = gammaOmega;
        state.muP(:,t,tt) = muPrior;
        fprintf('The %d-th iteration is over.\n',t);
    end
    xUAVOld = xUAV; Aold = A;TdelayOld = Tdelay;
    ...
    ddelay = -gradTdelay/norm(gradTdelay,2)/1e8;
    for nLS = 1:20
...
Hdx = sMDeriv.Phidx/norm(A,2);
Hdy = sMDeriv.Phidy/norm(A,2);
Hdz = sMDeriv.Phidz/norm(A,2);
