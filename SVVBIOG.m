function [xest,dPost, state] = SVVBIOG(yp,yd,Phip,Phid, inputParam)
...
xGrid = ones(T,1)*sMGParam.grid.x;
yGrid = ones(T,1)*sMGParam.grid.y;
...
%         pist(pist<eps) = eps;
    ...
    %% M-step
    lambdaIn = max(min(ones(N-1,1)*mean(st(2:end,:),1),1-eps),eps);
    % Off-Grid
    indNZ = find(mean(st(2:end,:),2)>=0.5);
%     indNZ = find(pis>0.5);
    if isempty(indNZ) 
        indNZ = 1:N-1;
    end
...
%         PhidSearchtmp = zeros(Md,1+length(indNZ),T);
        ...
        for iterArmijo = 1:iterArmijoMax
            ...
            yyGrid = yGrid + deltaY + armijoBeta^iterArmijo.*stepY;
            for tt = 1:T
                sMGParam.grid.x = xxGrid(tt,indNZ);
                sMGParam.grid.y = yyGrid(tt,indNZ);
                sMGParam.xUAVinput = xUAV(:,tt);
                sMGParam.subcarrInd = pilotSubcarrInd;
                sMGParam.symbol = ones(length(pilotSubcarrInd),1);
                [PhipSearchtmp(:,:,tt),derivPSearchtmp(tt)] = sensingMatrixGenWSymbolOG(sMGParam);
            ...
            if sum(f) <= sum(fBase) + armijoSigma*armijoAlpha*armijoBeta^iterArmijo*(trace(gx*stepX') + trace(gy*stepY')) ||...
                iterArmijo == iterArmijoMax
                Phip = PhipSearch;
                derivP = derivPSearch;
%                 xGrid = xxGrid;
...
%                     trace(Sx(:,:,tt)*(PhidSearchPostSquare(:,:,tt)) ))  );
    ...
    if norm(xest - xoldOut,'fro')^2/norm(xoldOut,'fro')^2 < 1e-4 && convBreak
        break;
    end
end
if JESD && ~convBreak
