...
%         if t == 1
        if 1
            alpha = maxStep./max(abs([gx,gy]),[],'all');
        end
...
%             ddeltaY(ddeltaY > maxStep) = maxStep; ddeltaY(ddeltaY < -maxStep) = -maxStep;
            ...
            [mergedGridOut] = mergeGrid([xxGrid;yyGrid],x2,xGrid,yGrid,sqrt(2)*maxStep);
...
%                 end
        ...
        if flagSL
            ...
            if 1
                alphaU   = min(1./abs([gxU,gyU]),[],'all')*0;
            ...
            steptau = -alpha*  gtau*1^(t-1)/c0;
%             stepxU  = -alphaU*  gxU *1^(t-1);
%             stepyU  = -alphaU*  gyU *1^(t-1);
%             stepzU  = -alphaU*  gzU *1^(t-1)*0;
%             steptau = -alphatau*  gtau*1^(t-1)/c0;
%             indNZ = 1:N;
            PhipSearchtmp = zeros(M,length(indNZ),numBS);
            for iterArmijo = 1:iterArmijoMax
                xUAVInput = xUAV     + deltaxUAV + armijoBeta2*armijoRho^(iterArmijo-1).*[stepxU;stepyU;stepzU];
        ...
        %                 sMGParam.symbol = ones(length(pilotSubcarrInd),1);
                ...
                if sum(f) <= sum(fBase) + armijoSigma*armijoBeta2*armijoRho^(iterArmijo-1)*(gxU*stepxU + gyU*stepyU + gzU*stepzU)...
                        + armijoSigma*armijoBeta3*armijoRho^(iterArmijo-1)*(c0*gtau*steptau) || iterArmijo == iterArmijoMax
